...
This guide covers the usage of the `az i18n` command suite for automated internationalization management.
...
- Displays a Rich table with ✓ (green) for present keys and ✗ (red) for missing ones.
...
**Key Features:**
- **Canary Validation:** Verifies translation integrity using hidden "canary" pairs.
- **Placeholder Preservation:** Ensures `{count}`, `{name}`, etc., are preserved verbatim.
- **Style Context:** Uses existing translations as few-shot examples for consistent tone.
- **Parallel Processing:** Translates all languages simultaneously for maximum speed.
...
## Troubleshooting
- **503 Unavailable:** The Gemini model is under high demand. Try again in a few minutes.
- **Canary validation failed:** The LLM output was malformed or incorrect. The tool will skip the affected locale to prevent file corruption.
- **Placeholder mismatch:** If a translated value is missing a variable like `{count}`, the tool will fall back to the English original and warn you.
