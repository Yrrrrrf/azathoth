...
console.print(
    Panel(
        Text(
            "🚀 Starting Time MCP Server... (Python Version)", style="bold green blink"
        ),
        border_style="green",
    )
)
...
@time_server.tool(description="Gets the current system time.")
def get_now() -> str:
    """
    ...
    console.print(
        Panel(
            Text(
                f"⏰ Time requested: {current_time} (Python Implementation)",
...
def main():
    ...
    console.print(
        Panel(
            Text(
                "✅ Server is ready with the 'get_now' tool. (Python Version)",
                style="bold green",
            ),
            border_style="green",
        )
