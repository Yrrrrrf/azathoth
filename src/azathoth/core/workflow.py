...
async def create_release(
    ...
    if t_code != 0:
        return GitResult(success=False, stdout=t_out, stderr=t_err, message="Tagging failed")
        
    p_code, p_out, p_err = await _run_git(["push", "origin", tag])
    if p_code != 0:
        return GitResult(success=False, stdout=p_out, stderr=p_err, message="Pushing tag failed")

    # Use gh CLI
    cmd = ["gh", "release", "create", tag, "--notes", notes, "--title", f"Release {tag}"]
    if is_prerelease:
